        ...
        rmBank(index) {
          ...
          this.$http.post(`/api/user/bank/remove/${bankID}`).then((res) => {
              if(res.data.length == 0) {
                ...
                this.$toast.error({title:'Error', message: `Error while removing your bank.. Please try again`});
                return;
              }

              this.$sound.play('success.mp3')
              this.$toast.success({title:'Success', message: `You successfully removed this bank`});
        ...
        addBankDetails() {
            if(this.checkFields(this.bankInputs)) {
                ...
                this.$http.post(`/api/user/bank/add`, {bank: bank}).then((res) => {
                  ...
                  if(res.id == undefined) {
                    this.$sound.play('error.mp3')
                    this.$toast.error({title:'Error', message: `Error while adding your bank.. Please try again`});
                    return;
                  }

                  bank.id = res.id
                  this.$sound.play('success.mp3')
                  this.$toast.success({title:'Success', message: `You successfully added a new bank to your list.`});
