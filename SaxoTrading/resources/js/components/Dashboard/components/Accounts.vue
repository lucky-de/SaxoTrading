  ...
  methods: {
    loginAccount(id, type) {
        ...
        this.$http.post(`/api/user/account/${id}`).then((res) => {
          ...
          this.$toast.error({title:'Error', message: `Something went wrong, please, try again`})
        })
    },
    ...
    createAccount(accountObj) {
      ...
      this.$http.post(`/api/user/create/account`, accountObj).then((res) => {
        if(res.data.id != undefined) {
          ...
          th.$toast.success({title:'Success', message: `You successfully created a new account.`})
        ...
        th.$toast.error({title:'Error', message: `We were unable to create you an account.`})
      })
    },
    editLeverage(index, newLeverage) {
      ...
      this.$http.post(`/api/user/update/account/leverage`, accountObj).then((res) => {
        if(res.data.id != undefined) {
          th.$sound.play('success.mp3')
          th.$toast.success({title:'Success', message: `Leverage updated successfully.`})
        ...
        th.$sound.play('error.mp3')
        th.$toast.error({title:'Error', message: `We were unable to update the leverage.`})
      })
    },
    addFunds(obj) {
      ...
      this.$http.post(`/api/user/accounts/${acc}/addfunds`, accountObj).then((res) => {
        if(res.data.id != undefined) {
          th.$sound.play('success.mp3')
          th.$toast.success({title:'Success', message: `Successfully added funds to an account.`})
        ...
        console.log(error)
        th.$sound.play('error.mp3')
        th.$toast.error({title:'Error', message: `We were unable to add funds to your account.`})
      })
