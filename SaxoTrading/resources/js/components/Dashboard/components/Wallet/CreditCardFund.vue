  ...
  methods: {
    fund() {
      ...
      if(!pass) {
        ...
        this.$toast.error({title:'Error', message: `All fields are required`})
        return
      }
      ...
      this.$http.post(`/api/user/fund`, obj).then((res) => {
        ...
        if(res.data.status == 'success') {
          ...
          this.$toast.success({title:'Success', message: `Your funding request was received at our end and awaits confirmation.`})
          this.$emit('return');
        } else {
          throw "Fail"
        }
