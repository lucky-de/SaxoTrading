    ...
    <div v-if="loaded" :class="`watch ${currStock.closed == true ? 'closed' : ''}`">
      <div class="watch-topbar d-flex w-100 mb-1 justify-content-between align-items-center">
        ...
        <div class="f-grow-half d-flex justify-content-center flex-column">
          <button class="btn-close-order" :class="order.pl >= 0 ? 'positive' : 'negative' " @click="closeOrder">Close</button>
      ...
      <div class="watch-body mt-1">
        <div class="sell-zone"  :style="{
          background: !isAskActive && !currStock.closed ? '#d81720 !important' : '',
      ...
      <div class="buy-zone" :style="{
        background: isAskActive && !currStock.closed ? '#19a337 !important' : '',
        color: isAskActive && !currStock.closed ? '#fff !important' : ''
      }"
      @click='changeSt(1)'
      >
  ...
  methods: {
    closeOrder() {
      ...
      this.$store.dispatch('closeOrder', this.order.order_id).then((res) => {
        ...
        th.$toast.success({title:'Success', message: `Closed position #${this.order.order_id} of ${this.order.stock.name}`})
        
        th.$store.dispatch('set_order', {id: this.order.order_id, order: res})
        th.hideModal();
      }).catch(error => {
        th.$sound.play('error.mp3')
        th.$toast.error({title:'Error', message: `Position #${this.order.order_id} of ${this.order.stock.name} can't be closed right now.`})
        console.log(error)
      })
    },
    ...
    trade() {
      ...
      this.$store.dispatch('editOrder', this.order).then((res) => {
        ...
        if(res == -1) {
          th.$sound.play('error.mp3')
          th.$toast.error({title: 'Error', message: `Couldn't find that order. Try again`})
        ...
        if(res == -2) {
          th.$sound.play('error.mp3')
          th.$toast.error({title: 'Error', message: `Insufficient balance for this order.`})
        ...
        if(res.id == undefined) {
          th.$sound.play('error.mp3')
          th.$toast.error({title: 'Error', message: `Unexpected error.`})
        ...
        th.$sound.play('success.mp3')
        th.$toast.success({title: 'Success!', message: `Order [PID:${th.order.order_id}] has been updated successfully!`})
