  ...
  methods: {
    ...
    addAlert(a) {
      ...
      this.$http.post(`/api/user/update/account/alerts`, { value : alerts }).then((res) => {
        this.$toast.info({title: 'Price Alert Created', message: `Created ${this.stock.name} ${a.alertOn.includes('bid') ? 'Bid' : 'Ask'} ≤ ${a.priceValue} price alert`})
      }).catch(error => {
        this.$toast.error({title: 'Error', message: `Your alert cannot be created right now`})
        this.loading = false
      })
    },
      ...
      switch(d.event)
      {
        case 'quick_order':
        {
          ...
          this.$store.dispatch('createOrder', order).then((res) => {
            ...
            this.$toast.success({title: 'Success', message: `${direction} ${this.stock.api_name} has been placed!`})
            ...
            this.$toast.error({title: 'Error', message: error.error})
