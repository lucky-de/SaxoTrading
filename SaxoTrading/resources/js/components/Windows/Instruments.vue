  ...
  methods: {
    ...
    itemMenuClicked(did) {
      ...
      if(did.args != undefined) {
        ...
        this.$store.dispatch('addtoFavs', {catid: did.args[0], symbol: stock}).then(() => {
                ...
                this.$toast.success({title:'Success!', message: `You added ${stock} to the watchlist`})
        }).catch(error => {
                this.$sound.play('error.mp3')
                this.$toast.error({title:'Error', message: `You have already added this instrument.`})
         })
      }
    ...
    order(direction, symbol) {
      ...
      this.$store.dispatch('createOrder', order).then((res) => {
        ...
        th.$toast.success({title: 'Success!', message: `${type} ${stock.name} has been placed!`})
        th.$notifAgent.push(`${type} ${stock.name} has been placed!`, 'positions');

        th.loadingSubmit = false
      }).catch(msg => {
        th.$sound.play('error.mp3')
        th.$toast.error({title: 'Error', message: msg.error})
