  ...
  <div class="h-100 pa-0">
    ...
    <div class="h-100" v-if="currTable == 'pos'">
      ...
      <template v-slot:item.actions="{ item }">
        <span @click="closeOrder(item.order_id)" :class="item.direction == 'Buy' ? 'buy' : 'sell'" v-html="actions"></span>
      </template>
  ...
  methods: {
    ...
    closeOrder(id) {
      ...
      this.$store.dispatch('closeOrder', id).then( (res) => {
        ...
        th.$toast.success({title:'Success', message: `Closed position ${id} of ${order.stock.name}`})
        th.$store.dispatch('set_order', {id: id, order: res})
      }).catch(error => {
        th.$sound.play('error.mp3')
        th.$toast.error({title:'Error', message: `Position ${id} of ${order.stock.name} can't be closed right now.`})
      })
    },
  },
